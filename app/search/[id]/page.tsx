...
function SearchPage({ params: { id } }: Props) {
  ...
  if (loading) return <h1 className='text-center p-10 animate-pulse text-xl text-indigo-600/50'>Loading Results...</h1>
    ...
    <div className='py-5'>
      <div className='flex items-center justify-between mb-7'>
        <div className='flex flex-col md:flex-row gap-x-4'>
          ...
          <p className='text-gray-300'>{snapshot.data()?.results?.length > 0 && `${snapshot.data()?.results.length} results found`}</p>
        </div>
        {deleteButton}
      </div>
